...
// Modelica.SIunits.MolarInternalEnergy Delta_h_solution_KCl = HeatOfSolution_KCl_Sanahuja1986(T) "[J/mol_NaCl]";
...
//  Modelica.SIunits.SpecificEnthalpy h_H2O =  Modelica.Media.Water.WaterIF97_base.specificEnthalpy_pT(p, T);
  Modelica.SIunits.SpecificEnthalpy h_Driesner = specificEnthalpy_pTX_Driesner(p, T, X[1]/(X[1]+X[end]));
...
//  Modelica.SIunits.Pressure p_check;
 ...
 BrineProp.Partial_Units.PartialMolarEnthalpy[:] H_appmol={(if b[i] > 0 then 
  ...
  BrineProp.Partial_Units.PartialMolarEnthalpy[:] H_appmol={0,
   ...
   0,0};
algorithm
//  Modelica.Utilities.Streams.print("specificEnthalpy_pTX_Driesner: "+String(h_Driesner)+" J/kg");
...
//Modelica.Utilities.Streams.print("b[CaCl2]: "+String(b[CaCl2])+" mol/kg");
...
//  Modelica.Utilities.Streams.print("int_cp_by_cpWater:"+Modelica.Math.Matrices.toString({int_cp_by_cpWater}));
