...
algorithm
  ...
  /*  y_H20 :=(1 - 2*molefractions[NaCl])*p_H2O/(phi_H2O*p)*exp(v_l_H2O*(p -
    ...
    phi_CH4 :=fugacity_CH4_Duan1992(p_gas, T);
    mu_l0_CH4_RT :=Par_CH4_Duan2006(
      p_gas,
      T,
      mu_l0_CH4_RT_c);
    lambda_CH4_Na :=Par_CH4_Duan2006(
      p_gas,
      T,
      lambda_CH4_Na_c);
    xi_CH4_NaCl :=Par_CH4_Duan2006(
      p_gas,
      T,
      xi_CH4_NaCl_c);
...
//      Modelica.Utilities.Streams.print("mola_CH4("+String(p_gas)+","+String(T-273.16)+")="+String(X_gas)+"->k="+String(X_gas/max(1,p_gas))+" (solubility_CH4_pTX_Duan2006)");
