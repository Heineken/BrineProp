...
//  Modelica.Utilities.Streams.print("eta_H2O= "+String(eta_H2O)+" Pa新");
  ...
  eta:= eta_H2O^X[end];
  Modelica.Utilities.Streams.print("eta_H2O^X[end]= "+String(eta_H2O)+"^"+String(X[end]) + " -> "+String(eta)+" Pa新");
...
//    Modelica.Utilities.Streams.print("etas["+String(i)+"]: "+String(etas[i]));
      eta:= eta*etas[i]^X[i]
        "geometric mean mixture rule (matrix multiplication)";
      Modelica.Utilities.Streams.print("Viscosity: "+String(etas[i])+"->"+String(eta)+" Pa新 (BrineProp.Viscosities.dynamicViscosity_Duan_pTX)");
    end if;
//    eta := eta + etas[i]*molalities[i];
  end for;
//  eta := etas*molalities[1:nX_salt]/(sum(molalities[1:nX_salt])) "linear mixing rule molality weighted (matrix multiplication)";
//  eta := prod(cat(1,{eta_H2O},etas).^X[1:nX_salt]) "geometric mean mixture rule (matrix multiplication)";

//  Modelica.Utilities.Streams.print("Viscosity("+String(p_Pa)+","+String(T_K)+"): "+String(eta)+" Pa新 (Partial_Viscosity_Phillips.dynamicViscosity_Duan_pTX)");
