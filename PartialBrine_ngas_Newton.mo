within Brine;
partial package PartialBrine_ngas_Newton
  "Template package for aqueous solutions of m Salts and n Gases, VLE solved by Newton's method"
...
constant String explicitVars = "ph"
 ...
 replaceable package Salt_data = Salt_Data;
...
constant FluidConstants[nS] BrineConstants(
     each chemicalFormula = "H2O+NaCl+KCl+CaCl2+MgCl2+SrCl2+CO2+N2",
     each structureFormula="H2O+NaCl+KCl+CaCl2+MgCl2+SrCl2+CO2+N2",
...
//  type Pressure_bar = Real(final quantity="Pressure", final unit="bar") "pressure in bar";
 ...
 //  (x,d,d_g,d_l,p_H2O,p_gas,X_l,p_degas,k)= quality_pTX(p_corr,T_corr,X);
   ...
   state = setState_pTX(p,T,X,phase);
   X_l=state.X_l;
   GVF=state.GVF;
   x=state.x;
   s=state.s;
...
*/
 ...
 //  input Modelica.SIunits.MolarMass MM[:]=fill(0,nX)     "molar masses of components";
  ...
  protected
     Brine.Partial_Units.Molality ionMoleFractions[nX];
  algorithm
  ...
  //  printVector(nM);
    assert(max(X)<=1 and min(X)>=0, "X out of range [0...1] = "+vector2string(X)+" (saturationPressure_H2O())");
    if X[end]>0 then
  ...
  function quality_pTX
    "finds the VLE iteratively by varying the normalized quantity of gas in the gasphase, calculates the densities"
  input Modelica.SIunits.Pressure p;
  input Modelica.SIunits.Temperature T_in "(min=273.15)";
  input Modelica.SIunits.MassFraction[nX] X "mass fraction, sum(X)=1";
  //input Modelica.SIunits.MassFraction m_CO2 "CO_2 insgesamt pro kg Fluid";
  input Real[nX_gas+1] n_g_start=fill(.5,nX_gas+1)
      "start value, all gas in gas phase, all water liquid";
  ...
  output Modelica.SIunits.Density d_l;
  output Modelica.SIunits.Pressure p_H2O;
  output Modelica.SIunits.Pressure[nX_gas] p_gas_out;
  output Modelica.SIunits.MassFraction[nX] X_l=X "start value";
  output Modelica.SIunits.Pressure p_degas;
  ...
  protected
    Modelica.SIunits.Pressure p_sat_H2O
      "= saturationPressure_H2O(p,T,X,MM_vec,nM_vec)";
  ...
  /*  Modelica.Utilities.Streams.print("quality_pTX("+String(p)+","+String(T)+","+vector2string(X_l[1:end],false)+")");
  ...
  X_l:=X;*/
  //  Modelica.Utilities.Streams.print("quality_pTX("+String(p)+","+String(T_in)+","+vector2string(X[1:end],false)+")");

    assert(max(X)<=1 and min(X)>=0, "X out of range [0...1] = "+vector2string(X)+" (quality_pTX())");
  //  assert(T>273.15,"T too low in PartialBrine_ngas_Newton.()");
  //    Modelica.Utilities.Streams.print("\nn_g_start=" + vector2string(n_g_start));

      if T_in<273.15 then
      Modelica.Utilities.Streams.print("T="+String(T_in)+" too low (<0°C), setting to 0°C in PartialBrine_ngas_Newton.quality_pTX()");
    end if;
    T:= max(273.16,T_in);

    p_sat_H2O := saturationPressure_H2O(p,T,X,MM_vec,nM_vec);
    p_degas := sum(saturationPressures(p,T,X,MM_vec)) + p_sat_H2O;

     if  p_degas< p then
  //    Modelica.Utilities.Streams.print("1Phase-Liquid (PartialBrine_Multi_TwoPhase_ngas.quality_pTX("+String(p)+","+String(T)+"))");
      x:=0;
      p_H2O := p_sat_H2O;
    else
      assert(max(X[end-nX_gas:end-1])>0,"Phase equilibrium cannot be calculated without dissolved gas ("+String(p/1e5)+" bar, "+String(T-273.15)+"°C).");
  ...
  //    n_g_norm :=n_g ./ n;
      ...
      //abbrechen wenn Druck-GG gefunden oder sehr geringer Gasanteil
        z:=z + 1;
        assert(z<=zmax,"Reached maximum number of iterations ("+String(z)+"/"+String(zmax)+") for solution equilibrium calculation. (quality_pTX("+String(p/1e5)+"bar,"+String(T-273.16)+"°C))\nDeltaP="+String(max(abs(p_sat-p_gas))));

  //     Modelica.Utilities.Streams.print("\nn_g_norm=" + vector2string(n_g_norm));
        n_g :=n_g_norm .* n;
...
*/
  ...
  //      Modelica.Utilities.Streams.print("\n"+String(z)+": x="+String(x)+" Delta_n_g="+String(max(abs(Delta_n_g_norm))));
   ...
   /*      Modelica.Utilities.Streams.print("n_l=" + vector2string(n_l));
      Modelica.Utilities.Streams.print("n_g=" + vector2string(n_g));
*/
    //PARTIAL PRESSURE
          p_gas := p * n_g/sum(n_g);
    ...
    //DEGASSING PRESSURE
          (p_H2O,p_H2O_0):=saturationPressure_H2O(p,T,X_l,MM_vec,nM_vec)
          "X_l ändert sich";
      if (p_H2O>p) then
          Modelica.Utilities.Streams.print("p_H2O(" + String(p/1e5) + "bar," +
            String(T - 273.15) + "°C, " + vector2string(X) + ") = "
  ...
  //        k:=solubilities_pTX(p=p, T=T, X_l=X_l, X=X, p_gas=fill(p/3,3)) ./ fill(p/3,3);
  //  Modelica.Utilities.Streams.print("X_l="+vector2string(X_l[nX_salt+1:end]));
          k:=solubilities_pTX(p=p, T=T, X_l=X_l, X=X, p_gas=p_gas[1:nX_gas]) ./ p_gas[1:nX_gas];
  //    Modelica.Utilities.Streams.print("k="+vector2string(k)+" (PartialBrine_ngas_Newton.quality_pTX)");
          ...
          f :=  p_gas-p_sat;
  //       Modelica.Utilities.Streams.print("p_gas=" + vector2string(p_gas) + "=>" + String(sum(p_gas)));
  //       Modelica.Utilities.Streams.print("p_sat=" + vector2string(p_sat));
...
//         Modelica.Utilities.Streams.print("Grad_f["+String(gamma)+",:] =" + vector2string(Grad_f[gamma,:]));
        end for;
*/

  //       Modelica.Utilities.Streams.print("k=" + vector2string(k));/**/
  //       Modelica.Utilities.Streams.print("dp_gas=" + vector2string(p_sat - p_gas));

    //SOLVE NEWTON STEP
  //        Delta_n_g_norm := Modelica.Math.Matrices.solve(Grad_f, -f)         "solve Grad_f*Delta_n_g_norm=-f";
  //        n_g_norm := n_g_norm + Delta_n_g_norm;
  //        Modelica.Utilities.Streams.print("Delta_n_g_norm="+vector2string(Delta_n_g_norm));
  ...
  //          end if;
      ...
      p_gas_out := p_gas[1:nX_gas];
    end if "p_degas< p";

  //  assert(x>=0 and (sum(n_gas_g) + n_H2O_g)>=0 or (not x>=0 and not (sum(n_gas_g) + n_H2O_g)>=0),"WEIRD!");
    d_g:= if x>0 then p/(Modelica.Constants.R*T)*(n_g*cat(1,MM_gas,{M_H2O}))/sum(n_g) else -1;

    d_l:=if not x<1 then -1 else density_liquid_pTX(p,T,X_l,MM_vec)
...
*/
  ...
  assert(p>0,"p="+String(p/1e5)+"bar - Negative pressure is not yet supported ;-) (PartialBrine_ngas_Newton.quality_pTX())");
  /*  Modelica.Utilities.Streams.print("quality_pTX("+String(p)+","+String(T2)+","+vector2string(X_l[1:end],false)+")");
  X[1:nX_salt] = X_[1:nX_salt];
  for i in nX_salt+1:nX-1 loop
    X[i]:=max(0,min(1e-3,X_[i]));
  end for;
  X[end]=1-sum(X[1:end-1]);
  X_l:=X;*/
  //  Modelica.Utilities.Streams.print("quality_pTX("+String(p)+","+String(T)+","+vector2string(X[1:end],false)+")");
...
*/
    ...
    //DEGASSING PRESSURE
      ...
      if (p_H2O>p) then
          Modelica.Utilities.Streams.print("p_H2O(" + String(p/1e5) + "bar," +
            String(T2 - 273.15) + "°C, " + vector2string(X) + ") = "
             + String(p_H2O/1e5) + "bar>p ! (PartialBrine_ngas_Newton.quality_pTX)");
        x:=1;
        break;
      end if;
